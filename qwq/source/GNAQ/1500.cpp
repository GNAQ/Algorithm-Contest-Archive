...
void Pushdown_Node(int _pos)//OK
{
	if (stag[_pos])
	{
...
int Get_Pos(int _pos,int _val)//OK
{
...
void Insert(int _pos,int len)//OK
{
	int _l=Get_Pos(troo,_pos),_r=Get_Pos(troo,_pos+1);
	Splay(_l,troo); Splay(_r,ch[_l][1]);
...
void Put_Cov(int _l,int _r,int _val)//OK
{
	_l=Get_Pos(troo,_l); _r=Get_Pos(troo,_r);
	Splay(_l,troo); Splay(_r,ch[_l][1]);
...
int Ask_Max() { return tmax[troo]; }//OK





/*int Split(int _l,int _r)
{
	_l=Get_Pos(troo,_l); _r=Get_Pos(troo,_l+_r+1);
...
int Build(int _l,int _r,int _fa)
{
    ...
    if (_l<mid) LCH=Build(_l,mid-1,mid);
    if (mid<_r) RCH=Build(mid+1,_r,mid);
...
void _Init()
{
	...
	for (int i=1;i<=1000008;i++) memque.push(i);
	troo=Build(1,n+2,0);
}
