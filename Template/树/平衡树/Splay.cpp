...
struct splay_tree { int fa,ch[2],valu,siz,w; } tree[100010];
int tsiz=0,troo=0;
int opt,typ,val;
...
inline void splay_ins(int& inx,int f)
{
	if (!inx) 
	{
		inx=++tsiz; tree[inx].fa=f; 
		tree[inx].siz=tree[inx].w=1;
		tree[inx].valu=val;
		splay(inx,0); return;
	}
	tree[inx].siz++;
	if (tree[inx].valu==val) { tree[inx].w++; splay(inx,0); return; }
	splay_ins(tree[inx].ch[(tree[inx].valu<val)],inx); return;
}
