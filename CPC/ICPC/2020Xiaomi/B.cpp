...
namespace TMP
{
	...
	* operator ==                    - checks if two Lines are same
...
void Is(int fx,int tx,double wx)
{
	at++;
	edge[at].pre=ptr[fx];
	edge[at].to=tx;
...
{
	Node cac1,cac2;
	for (int i=1;i<=n;i++) dis[i]=TMP::inf;
...
TMP::Point wallp[10010];
TMP::Line wall[10010];

int main()
{
	int n1,m1,k1;
	readx(n1); readx(m1); readx(k1);
	TMP::Point st,ed;
	...
	st.input(); ed.input();
	wallp[k1+k1+1]=st; wallp[k1+k1+2]=ed;
	
	TMP::Line l1;
	for (int i=1;i<=k1*2+2;i++)
	{
		l1.s=wallp[i];
		for (int j=1;j<i;j++)
		{
			...
			for (int l=1;l<=k1;l++) 
			{
				stat+=(l1.segcrossseg(wall[l])==2);
			}
			if (!stat) 
			{
				// printf("Connect %d %d with %.2lf\n",i,j,l1.length());
				Is(i,j, l1.length());
				m++;
			}
		}
