...
circle cirs[1010],big;
vector<Point> pts;
vector< pair<double, double> > curs;
int n,R; 

pair<Point, Point> intersection(Point &o1, double r1, Point &o2, double r2)
{
...
bool ChkCut(Point p)
{
	...
	for (int i=1;i<=n;i++) 
	{
		if (cirs[i].relation(p)==2) return false;
	}
	return true;
}
...
{
	readx(n); readx(R);
	big.r=R; big.p=Point(0,0);
	for (int i=1;i<=n;i++)
	{
		cirs[i].input();
	}
	
	int insnum=0; Point p1,p2;
	pair<Point, Point> ppnt;
	for (int i=1;i<=n;i++)
	{
		// insnum=big.pointcrosscircle(cirs[i], p1, p2);
		ppnt=intersection(big.p,big.r, cirs[i].p, cirs[i].r);
		p1=ppnt.first; p2=ppnt.second;
	...
	double ans=0;
	for (int i=0;i<siz1;i++)
		for (int j=i+1;j<siz1;j++)
			ans=max(ans,pts[i].distance(pts[j]));
	printf("%.11lf\n", ans);
	return;
}
