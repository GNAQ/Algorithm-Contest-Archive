...
inline void splayInsert(int i, int v)
{
    ...
    if (!rt[i]){
        rt[i] = x = ++ tot;
        dat[x] = v; sz[x] = cnt[x] = 1;
        fa[x] = ch[x][0] = ch[x][1] = 0;
        return;
    } int last = 0;
    while (1){
        if (dat[x] == v){ cnt[x] ++; pushup(last); break; }
        last = x;
        x = ch[x][v > dat[x]];
        if (!x){
            x = ++ tot; dat[x] = v; sz[x] = cnt[x] = 1;
...
inline int splayRank(int i, int v)//在第i棵splay中求比v小的数的个数
{
    int x = rt[i], ret = 0;
    while (x){
        if (dat[x] == v) return ret + ((ch[x][0])?sz[ch[x][0]]:0);
        if (dat[x] < v){
...
inline int splayFind(int i, int v)//在第i棵splay中找到值为v的节点并将它提升到根
{
    int x = rt[i];
    while (x){
        if (dat[x] == v){ splay(i, x, 0); return x; }
        x = ch[x][v > dat[x]];
    }
}
